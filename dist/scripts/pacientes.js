import { pacientes } from "../modeloDatos.js";
//renderizar pacientes cuando carge el doom
window.addEventListener("DOMContentLoaded", () => {
    renderPacientes();
});
//contenedor en de los pacientes
const contenedor = document.getElementById("pacientesContainer");
...
function buscarPaciente() {
    ...
    pacientes.forEach(paciente => {
        if (input.value == paciente.id.toString().toLowerCase() || input.value.toLowerCase() == paciente.nombre.toLowerCase()) {
            const card = document.createElement("div");
            card.className = `
            cardP p-4 rounded-[10px] text-left shadow-md w-[240px]
            flex flex-wrap w-[680px] gap-2
