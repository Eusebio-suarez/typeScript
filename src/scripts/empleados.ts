import { empleados, medicos, pacientes } from "../modeloDatos.js"

//obtener el contenedor de los empleados
const contenedor = document.getElementById("empleadosContainer") as HTMLDivElement
...
function renderizarMedicos():void{
    medicos.forEach(medico => {
    let card = document.createElement("div") as HTMLDivElement
    card.className = `
        cardP p-4 rounded-[10px] text-left shadow-md w-[240px]
        flex flex-wrap flex-col items-center  w-[350px] gap-2
        bg-white hover:border-b-[4px] border-[#0077b6] hover:scale-105 hover:shadow-xl duration-[300ms]
        `;
    card.innerHTML=`
        <div>
...
function renderizarEmpleados():void{
    empleados.forEach(empleado => {
        ...
        card.className = `
            cardP p-4 rounded-[10px] text-left shadow-md w-[240px]
            flex flex-wrap flex-col items-center justify-center w-[350px] gap-2
